...
awk 'FNR==1 && NR!=1 { while (/^<header>/) getline; } 1 {if (!a[$0]++) print}' ${workplace}/MetaPhlan_Results/*_profiled_mgm_labeled.txt > ${workplace}/MetaPhlan_Results/ABS_Proj_AllSamps_MetaPhlan_merged_abundance_table.txt
