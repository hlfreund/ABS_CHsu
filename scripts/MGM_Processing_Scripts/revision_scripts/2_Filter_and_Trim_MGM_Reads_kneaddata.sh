...
#SBATCH --output=Filter_and_Trim_Matched_ABS-HC_Reads_Kneaddata.stdout
#SBATCH --mail-user=lfreund@health.ucsd.edu
#SBATCH --mail-type=ALL
#SBATCH --job-name="Filter_and_Trim_Matched_ABS-HC_Reads_Kneaddata"
...
if [[ ! -d ${workplace}/ABS_Matched_HCs_Seqs/Kneaddata_Results ]]; then
    mkdir ${workplace}/ABS_Matched_HCs_Seqs/Kneaddata_Results

fi

# Kneaddata can trim the reads and remove host genome reads before taxonomic and functional annotation

for i in ${workplace}/ABS_Matched_HCs_Seqs/*_1.fastq;
do
    f=$(basename $i)
    SAMPLE=${f%_1.fastq}
    
    # if results dir called ${SAMPLE}_knead_files does not exist, then run kneaddata
    if [[ ! -d ${workplace}/ABS_Matched_HCs_Seqs/Kneaddata_Results/${SAMPLE}_knead_files ]]; then
        
        kneaddata --input1 ${workplace}/ABS_Matched_HCs_Seqs/${SAMPLE}_1.fastq --input2 ${workplace}/ABS_Matched_HCs_Seqs/${SAMPLE}_2.fastq -db ${knead_db} -t 4 --reorder --store-temp-output --output-prefix ${SAMPLE} --output ${workplace}/ABS_Matched_HCs_Seqs/Kneaddata_Results/${SAMPLE}_knead_files
        
        mv ${workplace}/ABS_Matched_HCs_Seqs/Kneaddata_Results/${SAMPLE}_knead_files/${SAMPLE}_paired_*.fastq ${workplace}/ABS_Matched_HCs_Seqs/Kneaddata_Results/
