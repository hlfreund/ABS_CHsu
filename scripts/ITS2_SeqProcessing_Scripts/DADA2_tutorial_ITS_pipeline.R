...
### Set the Path ####
path <- "/bigdata/aronsonlab/shared/LintonFreund/MSH_Seqs/Trimmed_Seqs/ITS2_Trimmed" # CHANGE ME to the directory containing the fastq files after unzipping.
...
load("DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
...
fnRs <- sort(list.files(path, pattern="_R2_clean.fastq", full.names = TRUE)); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")

fnFs

# Extract sample names, assuming filenames have format: SAMPLENAME_XXX.fastq
sample.names <- sapply(strsplit(basename(fnFs), "_R1"), `[`, 1); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata") #pattern where you want to split the name
sample.names
...
ggsave(plot1,filename = "DADA2_Results/ITS2_DADA2/its2_pretrim_DADA2_F_quality.pdf", width=15, height=12, dpi=600)
ggsave(plot2,filename = "DADA2_Results/ITS2_DADA2/its2_pretrim_DADA2_R_quality.pdf", width=15, height=12, dpi=600)
...
path
filtFs <- file.path(path, "DADA2_Results/ITS2_DADA2/Filtered", paste0(sample.names, "_F_filtered.fastq.gz"))
filtRs <- file.path(path, "DADA2_Results/ITS2_DADA2/Filtered", paste0(sample.names, "_R_filtered.fastq.gz"))
names(filtFs) <- sample.names
names(filtRs) <- sample.names; save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft=0,
                     ...
                     compress=TRUE, multithread=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata") # On Windows set multithread=FALSE
...
filtRs <- filtRs[file.exists(filtRs)] ; save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")

#### Learn the error rates ####

errF <- learnErrors(filtFs, multithread=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
errR <- learnErrors(filtRs, multithread=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
...
ggsave(plot_errorF,filename = "DADA2_Results/ITS2_DADA2/its2_errormodel_F_DADA2.pdf", width=15, height=15, dpi=600)
...
ggsave(plot_errorR,filename = "DADA2_Results/ITS2_DADA2/its2_errormodel_R_DADA2.pdf", width=15, height=15, dpi=600)
...
dadaFs <- dada(filtFs, err=errF, multithread=TRUE, pool=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata") # pseudo pooling is computationally more efficient but similar in results to pooling; pool = True will pool samples together before sample inference
dadaRs <- dada(filtRs, err=errR, multithread=TRUE, pool=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
...
seqtab <- makeSequenceTable(dadaFs); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
dim(seqtab)
...
compare_reads_plot = ggdensity(rowSums(seqtab), fill = "blue4", alpha = 0.7); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
pdf(file = "DADA2_Results/ITS2_DADA2/its2_compare_plots.pdf", height = 10, width = 12) # Make empty pdf
compare_reads_plot # Add plot to empty pdf
dev.off()


#### Remove Chimeras ####
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=TRUE, verbose=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
...
head(track)
write.table(track,"DADA2_Results/ITS2_DADA2/ITS2_tracking_reads_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE); save.image(file = "DADA2_Results/ITS2_DADA2/mydada_its2.Rdata")
...
write(asv_fasta, "DADA2_Results/ITS2_DADA2/ITS2_ASVs_dada2.fa") # write fasta file
write.table(asv_fasta,"DADA2_Results/ITS2_DADA2/ITS2_ASVs_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE)

# count table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "DADA2_Results/ITS2_DADA2/ITS2_ASVs_Counts_dada2_Updated.tsv", sep="\t", quote=F, col.names=NA)
write.table(asv_tab,"DADA2_Results/ITS2_DADA2/ITS2_ASVs_Counts_dada2_Updated.txt",sep="\t",row.names=TRUE,col.names=TRUE)

# tax table:
asv_tax <- taxa
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "DADA2_Results/ITS2_DADA2/ITS2_ASVs_Taxonomy_dada2.tsv", sep="\t", quote=F, col.names=NA)
write.table(asv_tax,"DADA2_Results/ITS2_DADA2/ITS2_ASVs_Taxonomy_dada2.txt",sep="\t",row.names=TRUE,col.names=TRUE)

#### Save as R objects ####
saveRDS(asv_tax, file = "DADA2_Results/ITS2_DADA2/ITS2_ASVs_Taxonomy_dada2_Robject.rds", ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
saveRDS(asv_tab, file = "DADA2_Results/ITS2_DADA2/ITS2_ASVs_Counts_dada2_Updated_Robject.rds", ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
saveRDS(asv_fasta, file = "DADA2_Results/ITS2_DADA2/ITS2_ASV_Sequences_dada2_Robject.rds", ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
