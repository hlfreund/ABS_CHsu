...
head(abs.HC.metadata)
abs.HC.metadata$Group_Color<-"lightgray"
...
#### Import ABS Metabolomics Data ####
# does not include data for all samples included here in this analysis!!!!!
# only considering metabolomics data for stool samples

# UC Davis Metabolomics Center explanation for data normalization:
# We usually provide first a variant of a ‘vector normalization’ in which we calculate the sum of all peak heights for all identified metabolites (but not the unknowns!) for each sample. We call such peak-sums “mTIC” in
...
# Data are ‘relative semi-quantifications’, meaning they are
...
#                        parameters), additional normalizations need to be performed

abs.metabolom<-as.data.frame(read_xlsx("data/ABS_metabolome_clean.xlsx", sheet="StoolOnly"))

# drop certain amine comparisons since they were enriched in this metabolomics run
drop.metabs<-c("Val_*","Tyr_*","Thr_*","Pro_*","Lys_*","Leu_*","Ile_*","Gln_*","gamma_Glutamyl_*","Asn_*",
               "Ala_*")

abs.metabolom2<-abs.metabolom[,!grepl(paste0(drop.metabs,collapse="|"), names(abs.metabolom))] # keeping only general paths, not species-specific pathways
abs.metabolom2[1:4,1:9] # metabolites start in column #8

abs.log.metab<-abs.metabolom2
abs.log.metab[,-c(1:7)]<-log(abs.log.metab[,-c(1:7)])
